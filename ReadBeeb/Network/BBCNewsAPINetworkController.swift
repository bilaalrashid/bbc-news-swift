...
struct BBCNewsAPINetworkController {
    ...
    static func fetchHomeTabData(postcode: String? = nil) async throws -> BBCNewsAPIHomeTabResult {
        let url: String = {
        ...
        let request = self.session.request(url).validate().serializingDecodable(BBCNewsAPIHomeTabResult.self)
